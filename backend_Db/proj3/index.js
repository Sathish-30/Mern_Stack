...
app.post("/register" , (req , res) => {
    const {fname , lname , address1 ,address2 , city , state , zip , country , gender } = req.body;
    if(!(fname && lname && address1 && address2 && city && state && zip && country && gender )){
        res.status(400).send({"error" : "Enter all valid credentials"});
    }else{
        res.status(400);
        res.sendFile(path.normalize(__dirname+"/views/index.html"));
    } 
    const query = "Insert Into contact_info values(? , ? , ? , ? , ? , ? , ? , ?)";
    db.query(query, [fname , lname , address1+address2 , city , state , zip , country , gender], (err) => {
        if (err) {
