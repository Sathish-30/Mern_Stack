...
//1. GET a random joke for random endpoint
...
app.get("/random/:id" , (req , res) => {
  ...
  const foundJoke = jokes.find(e => e.id === id);
  res.json(foundJoke);
})
...
app.get("/filter" , (req , res) => {
  ...
  const filteredJokes = jokes.filter(e => e.jokeType === jokeType);
...
app.put("/jokes/:id" , (req , res) => {
  ...
  const index = jokes.findIndex(e => e.id === id);
...
app.patch("/jokes/:id" , (req , res) => {
  ...
  const replacementJoke = {
    id, jokeText : jokeText || existingJoke.jokeText , jokeType : jokeType || existingJoke.jokeType
  }
  const index = jokes.findIndex(e => e.id === id);
  jokes[index] = replacementJoke;
  res.json(replacementJoke);
...
app.delete("/jokes/:id" , (req , res) => {
  const id = parseInt(req.params.id);
  const seacrhIndex = jokes.findIndex(e => e.id === id);
  if(seacrhIndex > -1){
    jokes.slice(seacrhIndex , 1);
    res.sendStatus(200);
  }else{
    res
      .status(404)
...
app.delete("/all" , (req , res) => {
    ...
    if(userKey === masterKey){
      console.log("Deleted all the records");
      jokes = [];
      res.json({msg : "Deleted all the records"}).status(100);
    }else{
